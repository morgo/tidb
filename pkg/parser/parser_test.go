...
func TestGeo(t *testing.T) {
	cases := []testCase{
		...
		// SPATIAL INDEX in CREATE TABLE
		{`CREATE TABLE t (id INT PRIMARY KEY, g GEOMETRY NOT NULL SRID 4326, SPATIAL INDEX idx_g (g))`, true, "CREATE TABLE `t` (`id` INT PRIMARY KEY,`g` GEOMETRY NOT NULL /*!80003 SRID 4326 */,SPATIAL `idx_g`(`g`))"},
		{`CREATE TABLE t (id INT PRIMARY KEY, g POINT NOT NULL, SPATIAL KEY idx_g (g))`, true, "CREATE TABLE `t` (`id` INT PRIMARY KEY,`g` POINT NOT NULL,SPATIAL `idx_g`(`g`))"},
		{`CREATE TABLE t (id INT PRIMARY KEY, g GEOMETRY NOT NULL, SPATIAL INDEX (g))`, true, "CREATE TABLE `t` (`id` INT PRIMARY KEY,`g` GEOMETRY NOT NULL,SPATIAL(`g`))"},

		// SPATIAL INDEX in ALTER TABLE
		{`ALTER TABLE t ADD SPATIAL INDEX idx_g (g)`, true, "ALTER TABLE `t` ADD SPATIAL `idx_g`(`g`)"},
		{`ALTER TABLE t ADD SPATIAL KEY idx_g (g)`, true, "ALTER TABLE `t` ADD SPATIAL `idx_g`(`g`)"},
		{`ALTER TABLE t ADD SPATIAL INDEX (g)`, true, "ALTER TABLE `t` ADD SPATIAL(`g`)"},

		// Combined ALTER: ADD COLUMN with GEOMETRY + ADD SPATIAL INDEX
		{`ALTER TABLE t ADD COLUMN g GEOMETRY GENERATED ALWAYS AS (ST_GeomFromText(wkt, 4326)) STORED NOT NULL SRID 4326, ADD SPATIAL INDEX idx_g (g)`, true, "ALTER TABLE `t` ADD COLUMN `g` GEOMETRY GENERATED ALWAYS AS(ST_GEOMFROMTEXT(`wkt`, 4326)) STORED NOT NULL /*!80003 SRID 4326 */, ADD SPATIAL `idx_g`(`g`)"},
	}
	RunTest(t, cases, false, false)
}
